...
**完整示例：**
```
IOT平台 2.0。设备与大屏数据未正确对应。预期结果：设备与大屏数据保持一致。测试负责人。C:\path\to\bug_screenshot.png,C:\path\to\detail.png
```
